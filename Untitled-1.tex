...
The system was implemented using Python 3.9.13, PyTorch 2.0.1, Ultralytics YOLOv8 8.0.196, OpenCV 4.8.0, NumPy 1.24.3, Matplotlib 3.7.1, and Pandas 2.0.3. 
Quality assessment uses blur sigmoid parameters $k = 10.0$ and $x_0 = 0.3$ with a variance cap of 500, brightness sensitivity of 1.0 with an optimal range of 100--140, and contrast sigmoid parameters $k = 8.0$ and $x_0 = 0.4$ with a standard deviation cap of 80. 
Model switching configuration includes a low threshold ($\tau_{low}$) of 0.32, a high threshold ($\tau_{high}$) of 0.36, a cooldown period of 10 frames, a frame check interval of 1, and the initial model set to YOLOv8n.
